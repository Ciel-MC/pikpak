  ...
  <div class="list-page list-page-files">
    ...
    <n-modal v-model:show="showUserMenu">
      <n-card style="width: 600px;" title="自定义菜单">
        <template #header>
          自定义菜单 <a href="https://www.tjsky.net/?p=220#i-8" target="_blank"> <n-icon style="vertical-align: middle;"
              size="20" color="#d03050"><zoom-question></zoom-question></n-icon> </a>
        </template>
        <template #header-extra>
...
const getFile = async (id: string) => {
  return await http.get('https://api-drive.mypikpak.com/drive/v1/files/' + id, {
    params: {
      _magic: '2021',
      thumbnail_size: 'SIZE_LARGE'
    }
...
const aria2Post = async (res: any, dir?: string) => {
  ...
  try {
    const response = await fetch(aria2Data.value.host, {
      method: 'POST',
      body: JSON.stringify(postData),
      headers: new Headers({
        'Content-Type': 'application/json'
